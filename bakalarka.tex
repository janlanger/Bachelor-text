...
Bohužel se mi tedy nepodařilo navíc takovou existující aplikaci, která by splnila všechny požadované podmínky, a proto jsem přistoupil k navržení a implementaci vlastního řešení
...
\item Použitá technologie WS umožní integraci služby s aplikacemi na běžně používaných platformách pro vývoj webových aplikací a plug-inů Studentovi berličky (PHP, C++, Java...).
\end{enumerate}
...
\subsubsection{dibi}
Dibi pochází stejně jako Nette Framework od českého programátora Davida Grudla. V jeho podtitulu najdeme slova \uv{tiny 'n' smart database layer} (malá a chytrá databázová vrstva). Jde o knihovnu, jejímž hlavním cílem je zjednodušit zápis SQL příkazů a ulehčit rutinní úlohy programátora, například získání výsledků jako dvourozměrné pole, vytvoření pole výskledků indexovaného podle klíče z databáze a podobně. Dibi se také stará o escapování jak identifikátorů tak dat v SQL dotazu, čímž chrání aplikaci proti SQL Injection. Lze ho použít jak jako nadstavbu PDO, tak přímo s nativními PHP rozšířeními jednotlivých databázových strojů.
