...
Následující odstavce obsahující pohled do historie KOSu. Na tomto místě bych rád upozornil, že vycházejí z několika málo zdrojů, které obsahují pouze kusé informace. Nejde tedy o v žádném případě o kompletní a pravděpodobně ani úplně přesnou historii, jde však o jediné informace, které se mi podařilo nalézt. Nicméně si myslím, že na krátké seznámení s historii tohoto informačního systému postačují.
...
Systém byl (a stále je) postaven nad databázovým strojem Oracle, uživatelské rozhraní bylo vygenerováno prostřednictvím Oracle Forms verze 4, které je přístupné prostřednictvím znakového terminálu. Zavedení systému značně zjednodušilo správu studijních agend a umožnilo studentům samostatně řešit některé úkoly bez nustnosti navštívit Pedagogické oddělení. Nedlouho po vytvoření systému jej zavedla také Fakulta strojní a později i zbylé fakulty ČVUT.

Nyní se přesuneme kousek blíže současnosti, do roku 2002. KOSu už je 10 let a nyní běží ve třech samostatných instancích - na FEL, FS a ve VIC pro ostatní fakulty. Za tu dobu se také ve velké míře rozšířili osobní počítače a rychlý internet do domácností, a tak sílí hlasy po možnosti přístupu přes WWW rozhraní. Je tedy vypsáno výběrové řízení na WWW rozhraní KOSu pro role Student a Ucitel. Zadání specifikovalo společné WWW rozhraní pro celé ČVUT, které bude komunikovat se všemi třemi instancemi databáze. Zároveň mělo být řešení postaveno na existujících základech a umožnit koexistenci s terminálovým přístupem. 

Zakázku vyhrála firma Trask Solutions, a.s. a webový KOS byl v září 2002 uveden do provozu, zatím jen nad instancí spravovanou ve VIC. K tomu uvádí Ing. Halaška\cite{student:komentar-ke-kos} \uv{Urychlené uvedení vybraných WWW formulářů do provozu k prvnímu září 2002 bylo možné (z našeho hlediska) pouze za cenu některých bezpečnostních kompromisů, kterých se my na FEL bojíme.} To se nakonec ukázalo jako správné rozhodnutí, rozhraní totiž obsahovalo množství závažných chyb, a tak se WWW KOS zpřístupnil pro studenty a učitele na FEL až v září 2003\cite{student:kos-na-web}.
...
Studijní informační systém KOS je uzavřený systém, ke kterému téměř nikdo nemá přímý aplikační přístup. Samotný KOS totiž neposkytuje žádné dostupné API, pomocí kterého by mohli aplikace jednotlivých kateder získat alespoň základní data o studentech, učitelích a vyučovaných předmětech. V podstatě jediným dnes dostupným způsobem jak tato data získat je soubor \textit{rz.xml}. Každý, kdo chce tyto informace ve své aplikaci použít, tak musí tento soubor nějakým způsobem zpracovat - typicky převést do podoby databáze se kterou pak pracuje samotná aplikace. V letním semestru 2009/2010 Jakub Jirůtka v rámci své bakalářské práce\cite{jirutka}  vytvořil KOSApi, webovou službu pro aplikační přístup k datům rozvrhu bez nutnosti parsovat \textit{rz.xml}. Nicméně tato práce začala již na jaře 2010, kdy KOSApi nebylo ještě dokončené, a proto jej tato práce přímo nevyužívá, ale implementuje vlastní parser pro \textit{rz.xml}.

\subsection{Současný způsob importu}
//TODO - zeptat se (asi) Hunky na podrobnosti 
